...
SELECT AVG(salary)
-- Find the average of salary of all employees
FROM EMPLOYEE;

SELECT AVG(salary)
-- Average salary of all Males
FROM Employee
Where sex='M';
...
-- AGGREGATION Technique - GROUP BY -- Count how many Males and Females are there. See this one and next 2 e.g.s
FROM Employee
GROUP BY sex;

SELECT SUM(total_sales), emp_id
-- Show sum of total sales by all employees
FROM Works_With
GROUP BY emp_id;


SELECT SUM(total_sales), client_id
-- Show over all total money spent by each client 	
FROM Works_With
GROUP BY client_id;
...
SELECT *
-- Find any client who has an LLC in its name
FROM client
WHERE client_name like '%LLC';
-- DEFINE a pattern with keyword 'Like' 


SELECT *
-- Find any employee born in October or Feb
FROM Employee
...
--  A Nested query is a which used multiple SELECT statements.
--  We use the result of one SELECT statement, to then use it as a feed for another SELECT statement
--  
...
-- ER : Entity Relational diagram: helps you define ,Visualise, organise and design the actual business database requirements into Database schemas. The ER diagram helps you identify the objects and the associated relationships and attributes these objects have amongst themselves. 
